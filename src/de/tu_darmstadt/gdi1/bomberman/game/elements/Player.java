...
public class Player extends GameElement
{
	...
	public boolean move (long ticknumber) {
		// Pr√ºfen: darf ich mich √ºberhaupt wieder bewegen?
		if (dir == direction.NULL || ticknumber < nextMoveAllowedTick)
			return false;

		// Die Richtung in Koordinaten √ºbersetzen
		...
		for (int i = 0; i < target.size()-1; i++) {
			if (target.get(i) instanceof PowerUp){
				...
				switch(up.getpowerupID()){
					case 1: 
						// Radius der Bomben wird erhˆht
						increaseBombRadius(); break;
					case 2:
						//Speed wird erhˆht
						increaseSpeed(); break;
					case 3:
						//Anzahl der legbaren Bomben wird erhˆht
